...
import { BookOpen, Compass, EyeOff, Heart, Lock, MapPin, Package, Puzzle, Route, ScrollText, Signpost, type LucideIcon } from "lucide-react";
                    ...
                    <Link href={`/map?mapId=${r.mapId}&route=${r.id}`} className="group flex items-center gap-3 py-2">
                      <span className="min-w-0 flex-1">
                        <span className="flex items-center gap-1.5 truncate font-sans text-sm text-parch/90 transition-colors group-hover:text-gold-bright">
                          {r.visibility === "private" && <Lock className="h-3 w-3 shrink-0 text-muted" aria-label={t("mapRoutePrivate")} />}
