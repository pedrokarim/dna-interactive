...
export function EventCalendar() {
  ...
  // timeZone UTC : nos dates ISO sont en UTC-minuit, on affiche le même jour partout.
  ...
  const shift = (days: number) => {
    setWindowStart((s) => addDaysIso(s, days));
    setSelected(null);
  };
  const goToday = () => {
    setWindowStart(defaultWindowStart(span));
    setSelected(null);
  };
  const changeZoom = (z: CalendarZoom) => {
    // garde le centre de la fenêtre courante
  ...
  return (
    <div className="relative overflow-hidden rounded-sm border border-line/20 bg-panel/60 p-4 sm:p-5">
      ...
      <div className="mb-3 flex flex-wrap items-center gap-2">
        <button type="button" aria-label="Période précédente" onClick={() => shift(-step)} className={ctrlBtn}>
        ...
        <button type="button" aria-label="Période suivante" onClick={() => shift(step)} className={ctrlBtn}>
          ...
          <div className="mb-2.5 flex items-center gap-3">
            ...
            <div className="relative h-4 flex-1">
              {ticks.map((t) => (
                <span
                  key={t.iso}
                  className="absolute -translate-x-1/2 font-mono text-[0.58rem] text-muted-2"
                  style={{ left: `${t.pct}%` }}
                >
